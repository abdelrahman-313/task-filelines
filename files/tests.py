...
from files.utils import random_line, read_lines  # just for type hints
...
class ViewTests(TestCase):
    ...
    def test_random_line_defaults_to_json(self, mock_random_line):
        obj = self._create_text_file()
        view = RandomLineView.as_view()

        # No explicit Accept -> defaults to application/json
    ...
    def test_longest_100_lines(self, mock_read_lines):
        # Create several UploadedFile objects; we’ll mock read_lines for each
        f1 = self._create_text_file(name="a.txt")
        f2 = self._create_text_file(name="b.txt")

        # Each call to read_lines returns a list; since the view calls it in a loop,
        # set side_effect to return values for f1 then f2
        ...
        # Ensure list sorted by length desc
        lengths = list(map(len, data))
        self.assertEqual(lengths, sorted(lengths, reverse=True))
        self.assertEqual(data[0], "y" * 200)  # the longest
    ...
    def test_longest_20_of_file(self, mock_read_lines):
        ...
        # Use 'pk' instead of 'id' if 'id' is not available
        ...
        self.assertEqual(data[0], "d" * 50)  # longest first
